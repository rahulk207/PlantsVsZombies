...
public class GameLostMenu {
    ...
    GameLostMenu(Pane gameinterfacepane, Stage primaryStage, ArrayList<Timeline> timelines, int leveNumber) throws FileNotFoundException {
        ...
        Text resumeGame = new Text("Game Over");
        ...
        Text restartGame = new Text("Restart Game");
        restartGame.setX(460); restartGame.setY(340);
        restartGame.setFont(Font.font("Comic Sans MS",  FontPosture.ITALIC,23));
        this.gameinterfacepane.getChildren().add(restartGame);

        Button restartGameButton = new Button();
        restartGameButton.setLayoutX(440); restartGameButton.setLayoutY(305);
        restartGameButton.setPrefWidth(170); restartGameButton.setPrefHeight(40);
        this.gameinterfacepane.getChildren().add(restartGameButton);

        restartGameButton.setOnMouseClicked(mouseEvent -> {
            try {
                Level level = new Level(leveNumber, primaryStage);
        ...
        {
            try {
                MainMenu mainMenu= new MainMenu(primaryStage,leveNumber);
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
        });

        restartGameButton.setOpacity(0);
